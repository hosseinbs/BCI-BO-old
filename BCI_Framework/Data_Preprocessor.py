...
#        print(os.getcwd())
    ...
    def extract_data_samples(self, subject, params_dict, optimal, fe_param, cutoff_frequencies_low_list = None, cutoff_frequencies_high_list = None):
        ...
        if cutoff_frequencies_low_list is None and self.feature_extractor_name != 'morlet':
            ...
            cutoff_frequencies_low = np.tile(np.array(cutoff_frequencies_low_list), (raw_X.shape[1],1))#np.reshape(cutoff_frequencies_low_list, (raw_X.shape[1],-1))
            cutoff_frequencies_high = np.tile(np.array(cutoff_frequencies_high_list), (raw_X.shape[1],1))#np.reshape(cutoff_frequencies_high_list, (raw_X.shape[1],-1))
