...
def write_results_to_file(input_matrix, write_type):
    if write_type == 'var':
        res_file = 'F:/bo_results/res_var' + Job_Params.feature_extraction +'.csv'
    elif write_type == 'mean':
        res_file = 'F:/bo_results/res_' + Job_Params.feature_extraction +'.csv'
    
    final_results = np.zeros((21,4))
    final_results[:,0] = input_matrix[:,0]
    final_results[:,1] = input_matrix[:,3]
    final_results[:,2] = input_matrix[:,7]
    final_results[:,3] = input_matrix[:,-2]
...
def write_results_to_file2(input_matrix_mean, input_matrix_var):
    ...
    res_file = 'F:/bo_results/results_' + Job_Params.feature_extraction +'.csv'
    input_matrix = np.char.array(np.around(input_matrix_mean, decimals=1)) + "$\pm$" + np.char.array(np.around(input_matrix_var, decimals=1)) + "                       " 
...
if __name__ == '__main__':
    ...
    framework_results = np.array([[80.5,82.39],[70.19,83.89],[74.26,78.15],[60.96,68.86],[56.33,58.37],[56.09,53.48],[94.79,94.79],[67.77,91.58],[75.7,82.87],
                     [53.02,72.84],[92.17,83.48],[77.55,86.12],[79,60.85],[61.13,54.42],[86.45,86.81],[73.68,41.23],[60.14,40.58],[56.74,26.98],
                     [87.36,56.32],[80.81,61.62],[83.71,39.39]])

    ##################################################input values##########################################################################
    N_runs = 10
    chooser_modules = ["GPEIOptChooser", "GPEIOptChooser1", "GPEIOptChooser2", "GPEIOptChooser3", "GPEIOptChooser4", "GPEIOptChooser5", "GPEIOptChooser6", "GPEIOptChooser7", "GPEIOptChooser8", "GPEIOptChooser9", 
                       "RandomForestEIChooser", "RandomForestEIChooser1", "RandomForestEIChooser2", "RandomForestEIChooser3", "RandomForestEIChooser4", "RandomForestEIChooser5", "RandomForestEIChooser6", "RandomForestEIChooser7", "RandomForestEIChooser8", "RandomForestEIChooser9",
                       "RandomChooser","RandomChooser1", "RandomChooser2", "RandomChooser3", "RandomChooser4", "RandomChooser5","RandomChooser6", "RandomChooser7", "RandomChooser8", "RandomChooser9"]
    
    datasets = ['BCICIII3b', 'BCICIV2b', 'BCICIV2a']
    accuracy_types = ['MLR', 'MIN', 'VOTE', 'AVERAGE']
    n_subject_all_data = 21
    ...
    #                     test_predictions[class_ind,:] = gp.predict(X_test)
                    ...
                    ######################################################AVERAGING########################################################################
                    accuracy_till_now = 100.0 * np.sum(np.argmax(np.array(sum_test_probs), axis = 0) == y_test)/float(len(y_test))
                    accuracy_till_now_train = 100.0 * np.sum(np.argmax(np.array(sum_train_probs), axis = 0) == y)/float(len(y))
...
#                 all_datasets_final_results_variances[n_subjects_processed:n_subjects_processed + config.configuration["number_of_subjects"], 4:8] = np.std(all_datasets_final_results_dict[chooser_module_for_dict], axis = 2)
        ...
        all_datasets_final_results[n_subjects_processed:n_subjects_processed + config.configuration["number_of_subjects"], :] = all_datasets_final_results[n_subjects_processed:n_subjects_processed + config.configuration["number_of_subjects"],:] / 10.0
...
#     write_results_to_file(all_datasets_final_results_variances, 'var')
    ...
    if Job_Params.feature_extraction == "BP":
        ...
        all_datasets_final_results_variances = np.column_stack((np.zeros((21, 1)), all_datasets_final_results_variances))
